...
st.caption("Power-Law Time Decay · Retention Elasticity Model · Corrected Button Logic")
...
def projected_hill_function(days_array, roas_array, ret_score, mode="iap"):
    ...
    if mode == "ad":
        # AD Multiplier (IAP'nin yaklaşık %75-80'i bandında, ret'e göre artar)
        final_mult = base_mult * ret_factor * (0.75 + (ret_score - 0.16) * 2)
    else:
        # IAP Penalty (%10 down-scale)
        final_mult = base_mult * ret_factor * 0.90

    ceiling_roas = last_roas * final_mult
    
    # 🔥 D720/D360 Elasticity Fix
    # 0.16 (40/20/10) skoru için h=1.2 -> 1.1x ratio
    # 0.205 (40/25/15) skoru için h=1.45 -> 1.2x ratio
